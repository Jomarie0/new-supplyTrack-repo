...
from apps.inventory.models import Product # Product is now imported directly from inventory
...
class ProductVariant(models.Model):
    ...
    @property
    def get_display_price(self): # Renamed property to avoid conflict with the new field
        return self.price if self.price is not None else self.product.price

    def save(self, *args, **kwargs):
        if not self.sku:
            self.sku = f"{self.product.id}-{self.size or 'N/A'}-{self.color or 'N/A'}"
...
class ProductReview(models.Model):
    ...
    rating = models.PositiveIntegerField(choices=[(i, str(i)) for i in range(1, 6)], help_text="Rating from 1 to 5 stars.")
    ...
    def __str__(self):
        customer_name = self.customer.user.username if self.customer and self.customer.user else "Unknown Customer"
        return f"Review for {self.product.name} by {customer_name} - {self.rating} stars"
